...
#SBATCH --job-name=ReSeq-StellarPGx-CYP2C19-30x-results # Name of job
#SBATCH --output=stdout-ReSeq-StellarPGx-CYP2C19-30x-results   # Standard output name
#SBATCH --error=stderror-ReSeq-StellarPGx-CYP2C19-30x-results    # Standard Errorname
...
RESULTS=${STAGING}Diplotype_Callers/StellarPGx/ReSeq_CYP2C19_diplotype_calls_30x/cyp2c19/alleles/
SUMMARY=${STAGING}Diplotype_Callers/StellarPGx/ReSeq_StellarPGx_CYP2C19_30x_summary.tsv
BAMFILES=${STAGING}ReSeq_CYP2C19_stellarpgx_diplotypes_30x/

# Initiate results file
echo -e "True-diplotype\tFull-diplotype\tCalled-diplotype\tCorrect\tCorrect-alleles\tMissed-alleles" > ${SUMMARY}
...
for diplo_bam in ${BAMFILES}CYP2C19_diplo_*.bam; do
    ...
    if [ -f ${RESULTS}${diplo_name}_cyp2c19.alleles ]
    then 
        called_dip=$( grep "*" ${RESULTS}${diplo_name}_cyp2c19.alleles | sed 's/[][]//g' )
    else
        called_dip=""
    fi
    ...
    # If one of the alleles is *36 or *37, also show the sub allele
    if [[ $allele1 == "36" || $allele1 == "37" || $allele2 == "36" || $allele2 == "37" ]]; then
        full_genotype=$geno_temp
    else
        full_genotype=""
    fi

    # Write to summary file
    echo -e "${true_dip}\t${full_genotype}\t${called_dip}\t${correct}\t${correct_alleles}\t${missed_alleles}" >> ${SUMMARY}
