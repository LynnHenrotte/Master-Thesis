...
#SBATCH --job-name=ReSeq-StellarPGx-CYP2C9-30x-results # Name of job
#SBATCH --output=stdout-ReSeq-StellarPGx-CYP2C9-30x-results   # Standard output name
#SBATCH --error=stderror-ReSeq-StellarPGx-CYP2C9-30x-results    # Standard Errorname
...
RESULTS=${STAGING}Diplotype_Callers/StellarPGx/ReSeq_CYP2C9_diplotype_calls_30x/cyp2c9/alleles/
SUMMARY=${STAGING}Diplotype_Callers/StellarPGx/ReSeq_StellarPGx_CYP2C9_30x_summary.tsv
...
do
    ...
    if [ -f ${RESULTS}CYP2C9_diplo_${allele1}_${allele2}_new_cyp2c9.alleles ]
    then 
        called_dip=$( grep "*" ${RESULTS}CYP2C9_diplo_${allele1}_${allele2}_new_cyp2c9.alleles | sed 's/[][]//g' )
    else
        called_dip=""
    fi
