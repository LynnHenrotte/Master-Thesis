...
#SBATCH --job-name=ReSeq-CYP2C9-all-get-qscores # Name of job
#SBATCH --output=stdout-ReSeq-CYP2C9-all-get-qscores   # Standard output name
#SBATCH --error=stderror-ReSeq-CYP2C9-all-get-qscores    # Standard Errorname
...
## Initiate file ##
SUMMARY=${DATA}Simulations_CYP2C9/ReSeq/Allelic_variant_qscores.tsv
...
for (( allele=2; allele<=85; allele++ )); do
    
    VCF=${IN_5X}ReSeq_CYP2C9_${allele}_compare_vcf/0001.vcf
...
for (( allele=2; allele<=85; allele++ )); do
    ...
    VCF=${IN_15X}ReSeq_CYP2C9_${allele}_compare_vcf/0001.vcf
...
for (( allele=2; allele<=85; allele++ )); do
    ...
    VCF=${IN_30X}ReSeq_CYP2C9_${allele}_compare_vcf/0001.vcf
